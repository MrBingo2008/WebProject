...
function webapp_ws_create(onopen) {
    ...
    if ('WebSocket' in window) {
        ...
        _grwebapp_websocket.onerror = function () {
            //alert("_grwebapp_websocket.onerror");
...
//参数：
//args：报表生成相关的参数，为一个结构体对象，各个数据项说明请参考帮助中“WEB报表(B/S报表)->WEB报表客户端->启动参数说明”部分
//report：报表模板文本，报表定义格式必须为JSON格式。此参数可空，为空表示执行非报表生成任务。
//data：报表数据文本，此参数可空。报表数据对应的 XML 或 JSON 文本，请参考帮助“WEB报表(B/S报表)->WEB报表数据”中的说明。 
...
//用ajax获取报表模板与报表数据，然后通过WebSocket发送相关数据给WEB报表客户端并生成报表
//参数既可以是单个对象，也可以是对象数组。如果是数组，表示成批生成多个报表
