...
public class TreeInterceptor extends EmptyInterceptor implements
	...
	public boolean onSave(Object entity, Serializable id, Object[] state,
		...
		if (entity instanceof NestedTree) {
			NestedTree<?> tree = (NestedTree<?>) entity;
	...
	public boolean onFlushDirty(Object entity, Serializable id,
		...
		for (int i = 0; i < propertyNames.length; i++) {
			if (propertyNames[i].equals(tree.getParentName())) {
				NestedTree<?> preParent = (NestedTree<?>) previousState[i];
				NestedTree<?> currParent = (NestedTree<?>) currentState[i];
	...
	private boolean updateParent(NestedTree<?> tree,
			NestedTree<?> preParent, NestedTree<?> currParent) {
		// 都为空、或都不为空且相等时，不作处理
